...
public class ImageOptionalAdapter extends RecyclerView.Adapter<ImageOptionalAdapter.ImageOptionalHolder>{
    // ViewModels
    ...
    public ImageOptionalAdapter(Activity activity) {
        this.activity = activity;
    }

    @NonNull
    @Override
    public ImageOptionalAdapter.ImageOptionalHolder onCreateViewHolder(@NonNull ViewGroup viewGroup, int viewType) {
        View view = LayoutInflater.from(activity).inflate(R.layout.single_item_image_optional, viewGroup, false);

        initializer(view);

        return new ImageOptionalAdapter.ImageOptionalHolder(view);
    }

    @Override
    public void onBindViewHolder(@NonNull ImageOptionalAdapter.ImageOptionalHolder holder, int i) {
        String answer = answers.get(i);

        if (Build.VERSION.SDK_INT > Build.VERSION_CODES.LOLLIPOP) {
            holder.itemView.setBackgroundResource(R.drawable.draw_16sdp_solid_white_border_quartz_ripple_solitude);
        }
        Picasso.get().load(answer+".png").into(holder.answerTextView);
    ...
    public class ImageOptionalHolder extends RecyclerView.ViewHolder {

        public TextView numberTextView;
        public ImageView answerTextView;

        public ImageOptionalHolder(@NonNull View view) {
            super(view);
            numberTextView = view.findViewById(R.id.single_item_image_optional_number_textView);
            answerTextView = view.findViewById(R.id.single_item_image_optional_answer_imageView);
        }
    }
}
